module Lattices
    """
        PeriodicSquareLatticeNeighbors(latt,pos)

        For a given position pos in a lattice latt, it returns an array of neighbor positions (represented as cartesian indexes) for an square lattice geometry. Supports dimensions up to 3.
    """
    function PeriodicSquareLatticeNeighbors(latt,pos;printLog=false)
        s=size(latt)
        dim=length(s)
        if dim==1
            v1=CartesianIndex(mod1(pos[1]-1,s[1]))
            v2=CartesianIndex(mod1(pos[1]+1,s[1]))
            if printLog
                println([v1 0 v2])
            end
            return [v1,v2]
        elseif dim==2
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2])
            v3 = CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
            v4 = CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
            if printLog
                println([0 v1 0; v3 0 v4 ; 0 v2 0])
            end
            return [v1,v2,v3,v4]
        elseif dim==3
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2], pos[3])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2], pos[3])
            v3 = CartesianIndex(pos[1], mod1(pos[2]-1,s[2]), pos[3])
            v4 = CartesianIndex(pos[1], mod1(pos[2]+1,s[2]), pos[3])
            v5 = CartesianIndex(pos[1], pos[2], mod1(pos[3]-1,s[3]))
            v6 = CartesianIndex(pos[1], pos[2], mod1(pos[3]+1,s[3]))
            if printLog
                println([0 v1 0; v3 0 v4 ; 0 v2 0])
                println([v5 0 v6])
            end
            return [v1,v2,v3,v4,v5,v6]
        else
            error("dimension not soported")
        end
    end

    """
        PeriodicTriangularLatticeNeighbors(latt,pos)

        For a given position pos in a lattice latt, it returns an array of neighbor positions (represented as cartesian indexes) for an triangular lattice geometry. Supports dimensions up to 3.
    """
    function PeriodicTriangularLatticeNeighbors(latt,pos;printLog=false)
        s=size(latt)
        dim=length(s)
        if dim==1
            v1=CartesianIndex(mod1(pos[1]-1,s[1]))
            v2=CartesianIndex(mod1(pos[1]+1,s[1]))
            if printLog
                println([v1 0 v2])
            end
            return [v1,v2]
        elseif dim==2
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2])
            v3 = CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
            v4 = CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
            v5 = CartesianIndex(mod1(pos[1]-1,s[1]),mod1(pos[2]+1,s[2]))
            v6 = CartesianIndex(mod1(pos[1]+1,s[1]),mod1(pos[2]+1,s[2]))
            if printLog
                println([0 v1 v5; v3 0 v4 ; 0 v2 v6])
            end
            return [v1,v2,v3,v4,v5,v6]
        elseif dim==3           
            #suposing that triangular 3D is the FCC close packing 
            #normal 2d neighbors
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2],pos[3])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2],pos[3])
            v3 = CartesianIndex(pos[1],mod1(pos[2]-1,s[2]),pos[3])
            v4 = CartesianIndex(pos[1],mod1(pos[2]+1,s[2]),pos[3])
            v5 = CartesianIndex(mod1(pos[1]-1,s[1]),mod1(pos[2]+1,s[2]),pos[3])
            v6 = CartesianIndex(mod1(pos[1]+1,s[1]),mod1(pos[2]+1,s[2]),pos[3])
            #upper neighbors
            v7 = CartesianIndex(pos[1],pos[2],mod1(pos[3]+1,s[3]))
            v8 = CartesianIndex(mod1(pos[1]-1,s[1]),pos[2],mod1(pos[3]+1,s[3]))
            v9 = CartesianIndex(mod1(pos[1]-1,s[1]),mod1(pos[2]+1,s[2]),mod1(pos[3]+1,s[3]))
            #lower neighbors
            v10 = CartesianIndex(pos[1],pos[2],mod1(pos[3]-1,s[3]))
            v11 = CartesianIndex(mod1(pos[1]-1,s[1]),pos[2],mod1(pos[3]-1,s[3]))
            v12 = CartesianIndex(mod1(pos[1]-1,s[1]),mod1(pos[2]+1,s[2]),mod1(pos[3]-1,s[3]))
            if printLog
                println([0 v1 v5; v3 0 v4 ; 0 v2 v6])
                println([0 v8 v9; 0 v7 0 ; 0 0 0 ])
                println([0 v11 v12; 0 v10 0 ; 0 0 0 ])
            end
            return [v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12]
        else
            error("dimension not soported")
        end
    end

    """
    PeriodicHexagonalLatticeNeighbors(latt,pos)

    For a given position pos in a lattice latt, it returns an array of neighbor positions (represented as cartesian indexes) for an hexagonal lattice geometry. Supports dimensions up to 2.

    """
    function PeriodicHexagonalLatticeNeighbors(latt,pos;printLog=false)
        s=size(latt)
        dim=length(s)
        if dim==1
            v1=CartesianIndex(mod1(pos[1]-1,s[1]))
            v2=CartesianIndex(mod1(pos[1]+1,s[1]))
            if printLog
                println([v1 0 v2])
            end
            return [v1,v2]
        elseif dim==2
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2])
            if mod(pos[1],2)==1
                if mod(pos[2],2)==1
                    v3=CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
                else
                    v3=CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
                end
            else
                if mod(pos[2],2)==0
                    v3=CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
                else
                    v3=CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
                end
            end

            if printLog
                println([v1,v2,v3])
            end
            return [v1,v2,v3]
        elseif dim==3           
            #suposing that hexagonal 3D is layered 2D hexagonal 
            #normal 2d neighbors
            v1 = CartesianIndex(mod1(pos[1]-1,s[1]), pos[2], pos[3])
            v2 = CartesianIndex(mod1(pos[1]+1,s[1]), pos[2], pos[3])
            if mod(pos[1],2)==1
                if mod(pos[2],2)==1
                    v3=CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
                else
                    v3=CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
                end
            else
                if mod(pos[2],2)==0
                    v3=CartesianIndex(pos[1],mod1(pos[2]+1,s[2]))
                else
                    v3=CartesianIndex(pos[1],mod1(pos[2]-1,s[2]))
                end
            end 
            v4 = CartesianIndex(pos[1], pos[2], mod1(pos[3]-1,s[3]))
            v5 = CartesianIndex(pos[1], pos[2], mod1(pos[3]+1,s[3]))
            if printLog
                println([v1,v2,v3,v2,v3])
            end
            return [v1,v2,v3,v4,v5]
        else
            error("dimension not soported")
        end
    end
    """
        Index`Lattice(latt,func)

        Creates a lattice of the same size than latt that in each site i has an array of the indexes j_1 , ... , j_n of the neighbors of i under the neighbor function `func`
    """
    function IndexLattice(latt,func;printLog=false)
        s=size(latt)
        fin=Array{Array{CartesianIndex{length(s)},1},length(s)}(undef,s)
        for pos in CartesianIndices(s)
            fin[pos]=func(latt,pos)
        end
        return fin
    end

    """
        ValueLattice(latt,func)

        Creates a lattice of the same size than latt that in each site i has an array of the values  latt[j_1] , ... , latt[j_n] of the neighbors j_1 , ... , j_n of under the neighbor function `func`
    """
    function ValueLattice(latt,func)
        s=size(latt)
        fin=Array{Array{Int8,1},length(s)}(s)
        for pos in CartesianIndices(s)
            idxs=func(latt,pos)
            for i in idxs
                push!(fin[pos],latt[i])
            end
        end
        return fin
    end
    """
    SumLattice(latt,func)

        Creates a lattice of the same size than latt that in each site i has the value sum_{<i,j>} latt[j] =  latt[j_1] + ... + latt[j_n] of the neighbors j_1 , ... , j_n of under the neighbor function `func`.
    """
    function SumLattice(latt,func)
        s=size(latt)
        fin=Array{Int8,length(s)}(s)
        for pos in CartesianIndices(s)
            idxs=func(latt,pos)
            for i in idxs
                fin[pos]+=latt[i]
            end
        end
        return fin
    end

    """
        BuildLattices(l,dim,g)

        Builds a l-side lattice of dimension `dim` and its neighbor index lattice with geometry `g`
    """
    function BuildLattices(metaParam,model)
        l=metaParam[1]
        dim=metaParam[2]
        g=metaParam[3]
        if ~(g in ["square","triangle","hexagonal"])
            error("Geometry string $g is not supported")
        end
        if model=="normal"
            pick=Array{Int8,1}([1,-1])
        else
            pick=Array{Int8,1}([1,0])
        end
        if dim==1
            latt=rand(pick,l)
        elseif dim==2
            latt=rand(pick,l,l)
        elseif dim==3
            latt=rand(pick,l,l,l)
        else
            error("Dimension not supported")
        end
        if g=="hexagonal"
            neigLatt=IndexLattice(latt,HexagonalLatticeNeighbors)
        elseif g=="triangle"
            neigLatt=IndexLattice(latt,TriangularLatticeNeighbors)
        else
            neigLatt=IndexLattice(latt,SquareLatticeNeighbors)
        end
        return latt , neigLatt
    end
end